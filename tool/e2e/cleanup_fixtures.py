...
assets={}
for pattern in ['NOW-010-fixture-*.json','NOW-010-custody-live-*.json','NOW-011-fixture-*.json','NOW-012-fixture-*.json','NOW-013-fixture-*.json','NOW-014-fixture-*.json']:
    for file in (root/'outputs').glob(pattern):
        item=json.loads(file.read_text(encoding='utf-8-sig'))
        assert item['marker'].startswith(('E2E-010','E2E-011','E2E-012','E2E-013','E2E-014'))
        if item.get('asset'):
...
print(f'PASS cleanup: {len(active_assets)} exact E2E-010/011/012/013/014 assets, {len(objects)} evidence objects; unrelated counts preserved')
