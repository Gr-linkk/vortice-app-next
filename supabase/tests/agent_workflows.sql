...
select pg_temp.ok((select value->'data'->>'review_required'='true' from results where name='assigned'),'assignment requires human review');
set local role authenticated;
select set_config('request.jwt.claim.sub','a0200000-0000-4000-8000-000000000001',true);
select public.review_agent_work_proposal((select (value->'data'->>'proposal_id')::uuid from results where name='assigned'),gen_random_uuid(),'apply');
set local role anon;
...
insert into results values('schedule_proposal',public.agent_execute(pg_temp.token('manage'),'schedule_work_order','{"work_order_id":"a0200000-0000-4000-8000-000000000030","revision":1,"assigned_to":"a0200000-0000-4000-8000-000000000004","due_date":"","planned_start":"2026-09-10T10:00:00Z","estimated_minutes":60,"priority":"normal","note":"Schedule service"}',gen_random_uuid()));
select pg_temp.ok((select value->'data'->>'review_required'='true' from results where name='schedule_proposal'),'agent proposes schedule');
set local role authenticated;
select public.review_agent_work_proposal((select (value->'data'->>'proposal_id')::uuid from results where name='schedule_proposal'),gen_random_uuid(),'apply');
set local role anon;
insert into results values('conflict_proposal',public.agent_execute(pg_temp.token('manage'),'schedule_work_order','{"work_order_id":"a0200000-0000-4000-8000-000000000031","revision":0,"assigned_to":"a0200000-0000-4000-8000-000000000004","due_date":"","planned_start":"2026-09-10T10:30:00Z","estimated_minutes":60,"priority":"normal","note":"Schedule conflict"}',gen_random_uuid()));
set local role authenticated;
select pg_temp.denied($q$select public.review_agent_work_proposal((select (value->'data'->>'proposal_id')::uuid from results where name='conflict_proposal'),gen_random_uuid(),'apply')$q$);
set local role anon;
insert into results values('scope_proposal',public.agent_execute(pg_temp.token('manage'),'edit_work_order','{"work_order_id":"a0200000-0000-4000-8000-000000000030","revision":2,"title":"Inspect cooling system","description":"Follow reviewed checklist","job_type":"inspection","expected_materials":"Coolant","priority":"high","note":"Clarify scope"}',gen_random_uuid()));
set local role authenticated;
select public.review_agent_work_proposal((select (value->'data'->>'proposal_id')::uuid from results where name='scope_proposal'),gen_random_uuid(),'apply');
set local role anon;
...
select pg_temp.ok((select assigned_to='a0200000-0000-4000-8000-000000000004'::uuid and title='Inspect cooling system' and status='assigned' from public.work_orders where id='a0200000-0000-4000-8000-000000000030'),'human-reviewed work state updated');
