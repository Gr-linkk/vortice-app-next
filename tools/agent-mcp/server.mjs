...
tools.push(
  ...
  workflow('work_order_context', 'Read component current hours, latest meter-log timestamps, per-task service-plan baselines, approved service history, open managed work, revisions, eligible assignees and published PM templates. Combine this equipment history with manuals when planning; ask the user to verify stale/missing readings. A recorded zero is not proof of a new machine or service. Baselines apply to the matching service task, never every task on a component. Lists are bounded (100 jobs/plans, 50 approved services); heed truncation flags.',
    { asset_id: uuid }, ['asset_id'], true),
  workflow('create_plan_draft', 'Propose a source-backed hours-based maintenance interval for a specific component. Does not create an active plan or invent service history. A person reviews and edits the proposal, supplies verified last-service hours and activates it in Vortice. Use work_order_context for component IDs. Optionally link a PM checklist draft from the same document; it must be reviewed and published before plan activation. Calendar-only or mixed calendar/hour conditions must be flagged in notes for human review; never silently convert or discard them. Preserve operation_id on retries.', {
    operation_id: uuid, document_id: uuid, asset_id: uuid, engine_id: uuid,
    interval_label: { type: 'string', minLength: 3, maxLength: 160 }, interval_hours: { type: 'integer', minimum: 1, maximum: 1000000 },
    source_page: { type: 'integer', minimum: 1, maximum: 30 }, source_quote: { type: 'string', minLength: 3, maxLength: 1000 },
  ...
  workflow('assign_work_order', 'Assign an existing managed work order to an eligible person. Requires explicit management permission. Use work_order_context for the current revision and assignee ID. Cannot sign, complete, approve or invoice work. Preserve operation_id on uncertain retries.',
    { operation_id: uuid, work_order_id: uuid, revision, assigned_to: uuid }, ['operation_id', 'work_order_id', 'revision', 'assigned_to']),
  workflow('schedule_work_order', 'Set the complete schedule for a managed work order with management permission. Supply all current values to preserve them. Empty strings clear the assignee, deadline or start; omitted duration clears duration. Overlapping bookings are rejected. Refresh current revision before changing; preserve operation_id on retries.', {
  ...
  workflow('edit_work_order', 'Edit full work-order scope before work starts, with management permission. Read current values first; supply all fields. Cannot change the equipment or pinned checklist. Refresh on a revision conflict. Preserve operation_id on retries.', {
