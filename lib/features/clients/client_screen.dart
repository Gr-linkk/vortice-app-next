...
class _TierSelectorState extends ConsumerState<_TierSelector> {
  ...
  Future<void> _saveTier(SubscriptionTier tier) async {
    ...
    final success = await ref.read(clientControllerProvider.notifier).updateClient(
        widget.client.id, {'subscription_tier': tier.value});
    if (success && mounted) {
    ...
    return Column(
      ...
      children: [
        ...
        Wrap(
          spacing: 8,
          runSpacing: 8,
          children: SubscriptionTier.values.where((t) => t != SubscriptionTier.predictive).map((tier) {
      ...
      children: [
        ...
        assetsAsync.when(
          ...
          data: (assets) {
            ...
            return Column(
              children: assets.map((asset) => InkWell(
...
class _OrgSection extends ConsumerWidget {
            ...
            data: (org) => Row(
              children: [
                ...
                IconButton(
                  ...
                  tooltip: 'Rename',
                  onPressed: () => _showRenameOrgDialog(context, ref, orgId!, org?.name ?? ''),
                ),
              ],
            ),
          ),
  ...
  void _showRenameOrgDialog(BuildContext context, WidgetRef ref, String orgId, String currentName) {
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              Navigator.pop(ctx);
              await ref.read(orgControllerProvider.notifier).renameOrg(orgId, name);
