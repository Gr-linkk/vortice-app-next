...
final workOrderByIdProvider = FutureProvider.family<WorkOrder?, String>((
  ref,
  id,
) async {
...
final assetNameProvider = FutureProvider.family<String?, String>((
  ref,
  assetId,
) async {
  if (await ref.watch(profileProvider.future) == null) return null;
...
final latestEngineHoursProvider =
    FutureProvider.family<EngineHoursSnapshot, String>((ref, engineId) async {
      final rows = await supabase
          ...
          .select(
            'id, title, hours_at_start, hours_at_end, updated_at, created_at',
          )
...
final workOrderAssignmentsProvider =
    FutureProvider.family<List<WorkOrderAssignment>, String>((
      ...
      return (data as List)
          .map(
            (row) => WorkOrderAssignment.fromJson(row as Map<String, dynamic>),
          )
          .toList();
    });
    ...
    FutureProvider.family<bool, String>((ref, workOrderId) async {
      ...
      try {
        final assignments = await ref.watch(
          workOrderAssignmentsProvider(workOrderId).future,
        );
        if (assignments.any(
      ...
      try {
        final workOrder = await ref.watch(
          workOrderByIdProvider(workOrderId).future,
        );
